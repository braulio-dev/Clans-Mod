...
// Credit to this github user
// https://github.com/SerenityEnterprises/SerenityCE/blob/master/src/main/java/host/serenity/serenity/util/render/TTFFontRenderer.java
...
public class CustomFontRenderer {
    ...
    public CustomFontRenderer(final Font font, final int size) {
        this(font, size, 256);
    }

    public CustomFontRenderer(final Font font, final int size, final int characterCount) {
        this(font, size, characterCount, true);
    }

    public CustomFontRenderer(final Font font, final int size, final boolean fractionalMetrics) {
        this(font, size, 256, fractionalMetrics);
    }

    public CustomFontRenderer(final Font font, final int size, final int characterCount,
                              final boolean fractionalMetrics) {
        java.awt.Font fontTo;
        try {
            final InputStream path = CustomFontRenderer.class.getResourceAsStream(font.getPath());
            fontTo = java.awt.Font.createFont(java.awt.Font.TRUETYPE_FONT, path);
        ...
        this.regularData = this.setup(new CharacterData[characterCount], java.awt.Font.PLAIN, size);
        this.boldData = this.setup(new CharacterData[characterCount], java.awt.Font.BOLD, size);
        this.italicsData = this.setup(new CharacterData[characterCount], java.awt.Font.ITALIC, size);
    }
     ...
     */
    private CharacterData[] setup(final CharacterData[] characterData, final int type, final int size) {
        // Quickly generates the colors.
        this.generateColors();

        // Changes the type of the font to the given type.
        final java.awt.Font font = this.font.deriveFont(type, size);
        ...
        for (int index = 0; index < characterData.length; index++) {
            ...
            final BufferedImage characterImage =
                    new BufferedImage(MathHelper.ceiling_double_int(width), MathHelper.ceiling_double_int(height),
            ...
            characterData[index] =
                    new CharacterData(character, characterImage.getWidth(), characterImage.getHeight(), textureId);
        }
    ...
    private void createTexture(final int textureId, final BufferedImage image) {
        ...
        final ByteBuffer buffer =
                BufferUtils.createByteBuffer(image.getWidth() * image.getHeight() * 4); //4 for RGBA, 3 for RGB
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (character == COLOR_INVOKER && i < length) {
                ...
                if (index < 16) {
                    ...
                    GL11.glColor4d((textColor >> 16) / 255d, (textColor >> 8 & 255) / 255d, (textColor & 255) / 255d,
                                   a
                    );
                } else if (index == 16) {
                    obfuscated = true;
                ...
                if (strikethrough) {
                    this.drawLine(new Vector2f(0, charData.height / 2f),
                                  new Vector2f(charData.width, charData.height / 2f), 3
                    );
                }

                // Draws the underline if enabled.
                if (underlined) {
                    this.drawLine(new Vector2f(0, charData.height - 15),
                                  new Vector2f(charData.width, charData.height - 15), 3
                    );
                }
     ...
     */
    private void drawChar(final char character, final CharacterData[] characterData, float x, float y) {
    ...
    public void drawCenteredString(final String displayString, final int xPosition, final int yPosition,
                                   final int color) {
        this.drawString(displayString, xPosition - this.getWidth(displayString) / 2, yPosition, color);
    }


    /**
     * @author Rey
     * All registered fonts for the client
     */
    public enum Font {

        OPEN_SANS_LIGHT("/assets/clansmod/fonts/OpenSans-Light.ttf"),
        OPEN_SANS_BOLD("/assets/clansmod/fonts/OpenSans-Bold.ttf"),
        OPEN_SANS_SEMI_BOLD("/assets/clansmod/fonts/OpenSans-SemiBold.ttf");
